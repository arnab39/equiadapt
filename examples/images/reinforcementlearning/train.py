...
def select_action(dqn, state, steps_done, exp_hyperparams, stop_training):
    ...
    eps_threshold = exp_hyperparams["eps_end"] + (
        exp_hyperparams["eps_start"] - exp_hyperparams["eps_end"]
    ) * math.exp(-1.0 * steps_done / exp_hyperparams["eps_decay"])
...
def optimize_model(
    ...
    next_state_values[non_final_mask] = (
        target_dqn(non_final_next_states).max(1)[0].detach()
    )
...
def train_rl(hyperparams: DictConfig) -> None:
    ...
    mean_last = deque(
        [0] * exp_hyperparams["last_episodes_num"], exp_hyperparams["last_episodes_num"]
    )
    ...
    for i_episode in tqdm(range(num_episodes), desc="Training Episodes"):
        ...
        for t in count():
            ...
            action = select_action(
                dqn, state, steps_done, exp_hyperparams, stop_training
            )
