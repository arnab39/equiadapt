...
class E_GCL(nn.Module):
    ...
    def __init__(
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(hidden_dim + input_nf + nodes_att_dim, hidden_dim),
            act_fn,
            nn.Linear(hidden_dim, output_nf),
        )

        layer = nn.Linear(
            hidden_dim, num_vectors_in * num_vectors_out, bias=False
        )  # outputs a scalar
        torch.nn.init.xavier_uniform_(layer.weight, gain=0.001)
    ...
    def edge_model(self, source, target, radial, edge_attr):
        ...
        if edge_attr is None:  # Unused.
            ...
            )  # concatenates inputs to be passed into phi_e
        out = self.edge_mlp(
            out
        )  # phi_e from eqn. (3). Shape: (n_nodes * batch_size) x hidden_dim
        if self.attention:
    ...
    def node_model(self, h, edge_index, edge_attr, node_attr):
        ...
        agg = unsorted_segment_sum(
            edge_attr, row, num_segments=h.size(0)
        )  # (n_nodes * batch_size) x hidden_dim. m_i from paper.
        if node_attr is not None:
            agg = torch.cat([h, agg, node_attr], dim=1)
        else:
            agg = torch.cat(
                [h, agg], dim=1
            )  # concatenate inputs for phi_h. (n_nodes * batch_size) x (2*hidden_dim)
    ...
    def coord_model(self, coord, edge_index, coord_diff, radial, edge_feat):
        ...
        if coord_diff.dim() == 2:
            ...
            coord = coord.unsqueeze(2).repeat(
                1, 1, self.num_vectors_out
            )  # n_nodes * batch_size x coord_dim x num_vectors_out
        # coord_diff = coord_diff / radial.unsqueeze(1)
        #
        trans = torch.einsum(
            "bij,bci->bcj", coord_matrix, coord_diff
        )  # (n_edges * batch_size) x coord_dim x 1
        trans = torch.clamp(
            trans, min=-100, max=100
        )  # This is never activated but just in case it case it explosed it may save the train
        agg = unsorted_segment_mean(
            trans, row, num_segments=coord.size(0)
        )  # n_nodes * batch_size x coord_dim x 1. sum from eqn. (4)
        if self.last_layer:
            coord = coord.mean(dim=2, keepdim=True) + agg * self.coords_weight
        else:
            coord += (
                agg * self.coords_weight
            )  # Update coordinate embeddings following eqn. (4)
        return coord  #
    ...
    def coord2radial(self, edge_index, coord):
        ...
        coord_diff = (
            coord[row] - coord[col]
        )  # differences between cords of adjacent nodes. Shape: (batch_size * n_edges) x coord_dim
        radial = torch.sum((coord_diff) ** 2, 1).unsqueeze(
            1
        )  # squared distances. Shape: (n_edges * batch_size) x 1
    ...
    def forward(self, h, edge_index, coord, edge_attr=None, node_attr=None):
        ...
        )  # Shape: (n_edges * batch_size) x hidden_dim
        coord = self.coord_model(
            coord, edge_index, coord_diff, radial, edge_feat
        )  # Updated coord embeddings from eqn. 4. (n_nodes * batch_size) x coord_dim x 1
...
class E_GCL_vel(E_GCL):
    ...
    def __init__(
        ...
        self.coord_mlp_vel = nn.Sequential(
            nn.Linear(input_nf, hidden_dim),
            act_fn,
            nn.Linear(hidden_dim, num_vectors_in * num_vectors_out),
        )
    ...
    def forward(self, h, edge_index, coord, vel, edge_attr=None, node_attr=None):
        ...
        coord_vel_matrix = self.coord_mlp_vel(h).view(
            -1, self.num_vectors_in, self.num_vectors_out
        )
        if vel.dim() == 2:
            vel = vel.unsqueeze(2)
