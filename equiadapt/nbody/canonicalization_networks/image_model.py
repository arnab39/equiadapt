...
from canonical_network.models.image_networks import (
    VanillaNetwork,
...
from canonical_network.utils import (
    ...
    check_rotoreflection_invariance,
...
class LitClassifier(pl.LightningModule):
    def __init__(self, hyperparams):
        ...
        if hyperparams.model == "vanilla":
            ...
            self.network = OptimizationCanonizationNetwork(
                self.encoder, self.im_shape, num_classes, hyperparams
            )
        else:
        ...
        if self.hyperparams.model in ("equivariant", "canonized_pca"):
            if self.hyperparams.check_invariance:
                if self.hyperparams.group_type == "roto-reflection":
                    self.num_batches_invariant += check_rotoreflection_invariance(
                        self.network, x, self.hyperparams.num_rotations
                    )
                elif self.hyperparams.group_type == "rotation":
                    self.num_batches_invariant += check_rotation_invariance(
                        self.network, x, self.hyperparams.num_rotations
                    )
                else:
            ...
            if batch_idx == 0:
                ...
                self.canonized_image_buffer = torch.cat(
                    (self.canonized_image_buffer, canonized_images), dim=0
                )
                self.y_buffer = torch.cat((self.y_buffer, y), dim=0)
                if (
                    self.image_buffer.shape[0] > 2000
                    and self.hyperparams.save_canonized_images
                ):
                    save_images_class_wise(
                        self.image_buffer,
                        self.y_buffer,
                        "./canonical_network/visualization/"
                        + self.hyperparams.dataset
                        + "/"
                        + self.hyperparams.model
                        + "/kernel_"
                        + str(self.hyperparams.canonization_kernel_size)
                        + "_num_layers_"
                        + str(self.hyperparams.canonization_num_layers)
                        + "_"
                        + self.hyperparams.group_type
                        + "_"
                        + str(self.hyperparams.num_rotations),
                        "original_images",
                        self.num_classes,
                    )
                    save_images_class_wise(
    ...
    def on_test_end(self):
        if (
            self.hyperparams.model == "equivariant"
            and self.hyperparams.check_invariance
        ):
            print(
                f"Fraction of batches which are {self.hyperparams.group_type} "
    ...
    def configure_optimizers(self):
        if (
            "resnet" in self.hyperparams.base_encoder
            and "mnist" not in self.hyperparams.dataset
        ):
