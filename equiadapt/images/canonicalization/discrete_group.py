...
class DiscreteGroupImageCanonicalization(DiscreteGroupCanonicalization):
    def __init__(
        ...
        self.pad = (
            torch.nn.Identity()
            if is_grayscale
            else transforms.Pad(math.ceil(in_shape[-2] * 0.4), padding_mode='edge')
        )
        self.crop = (
            torch.nn.Identity()
            if is_grayscale
        ...
        if targets:
            ...
            for t in range(len(targets['boxes'])):
                targets[t]['boxes'] = rotate_boxes(
                    targets[t]['boxes'], group_element_dict['rotation'], image_width
                )
                targets[t]['masks'] = rotate_masks(
                    targets[t]['masks'], -group_element_dict['rotation']
                )
    ...
    def invert_canonicalization(
        self, x_canonicalized_out: torch.Tensor, induced_rep_type: str = 'regular'
    ):
        """
