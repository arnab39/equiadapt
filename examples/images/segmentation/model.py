...
class ImageSegmentationPipeline(pl.LightningModule):
    def __init__(self, hyperparams: DictConfig):
        ...
        self.loss, self.image_shape, self.num_classes = get_dataset_specific_info(hyperparams.dataset.dataset_name, hyperparams.prediction.prediction_network_architecture)
    ...
    def apply_loss(self, loss_dict: dict, pred_masks: torch.Tensor, targets_canonicalized: dict, iou_predictions: torch.Tensor = None):
        assert self.loss or loss_dict, "Either pass a loss function or a dictionary of pre-computed losses for segmentation task loss"
        ...
        loss_focal = torch.tensor(0., device=self.hyperparams.device)
        loss_dice = torch.tensor(0., device=self.hyperparams.device)
        loss_iou = torch.tensor(0., device=self.hyperparams.device)
        
        for pred_mask, target, iou_prediction in zip(pred_masks, targets_canonicalized, iou_predictions):
            ...
            for loss_func in self.loss:
                assert hasattr(loss_func, 'forward'), "The loss function must have a forward method"
                if loss_func.name == 'focal_loss': loss_focal += loss_func(pred_mask, gt_mask.float(), num_masks)
                elif loss_func.name == 'dice_loss': loss_dice += loss_func(pred_mask, gt_mask, num_masks)
                else: raise ValueError(f"Loss function {loss_func.name} is not supported")
            
            
            if iou_predictions:
                batch_iou = calc_iou(pred_mask, gt_mask)
                loss_iou += torch.nn.functional.mse_loss(iou_prediction, batch_iou, reduction='sum') / num_masks
    ...
    def training_step(self, batch: torch.Tensor):
        ...
        if self.hyperparams.experiment.training.loss.task_weight: 
            ...
            loss_dict, pred_masks, iou_predictions, _ = self.prediction_network(x_canonicalized, targets_canonicalized)
            ...
            task_loss = self.apply_loss(loss_dict, pred_masks, targets_canonicalized, iou_predictions)
    ...
    def validation_step(self, batch: torch.Tensor):
        ...
        # Meanwhile, Segment-Anything (SAM) can return boxes, ious, masks predictions 
        # For uniformity, we will ensure the prediction network returns both losses and predictions irrespective of the model
        _, _, _, outputs = self.prediction_network(x_canonicalized, targets_canonicalized)
        ...
        if self.hyperparams.experiment.training.loss.prior_weight:
            metric_identity = self.canonicalizer.get_identity_metric()
            validation_metrics.update({
                    "val/identity_metric": metric_identity
                })
