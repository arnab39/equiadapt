...
class CIFAR100DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            ...
            # print('Valid dataset size: ', len(self.valid_dataset))
            # Not a good strategy for splitting data but most papers use this
            self.train_dataset = CIFAR100(self.data_path, train=True, transform=self.train_transform, download=True)
            self.valid_dataset = CIFAR100(self.data_path, train=False, transform=self.test_transform, download=True)
        if stage == "test":
            self.test_dataset = CIFAR100(self.data_path, train=False, transform=self.test_transform, download=True)
            print('Test dataset size: ', len(self.test_dataset))
