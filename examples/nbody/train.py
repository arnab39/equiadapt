...
def train_nbody():
    ...
    checkpoint_callback = ModelCheckpoint(dirpath="canonical_network/results/nbody/model_saves", filename= hyperparams.model + "_" + wandb.run.name + "_{epoch}_{valid/loss:.3f}", monitor="valid/loss", mode="min")
    ...
    callbacks = [checkpoint_callback, early_stop_lr_callback, early_stop_metric_callback] if hyperparams.checkpoint else [early_stop_lr_callback, early_stop_metric_callback]

    model = NBodyPipeline(hyperparams)
